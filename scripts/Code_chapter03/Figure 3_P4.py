...
OUTPUT_DIR = RESULTS_ROOT / "04_cluster_biology_HCT116"
...
DATA_FILE = (
    DATA_ROOT / "HCT116_315drugs_log2FC_8327_for_UMAP.csv"
)
MAPPING_FILE = DATA_ROOT / "uniprot_to_gene_cache_HCT116.csv"
LABEL_FILE = DATA_ROOT / "01_final_k9_labels_HCT116.csv"
...
# =============================================================
#  STEP 0: Load Local Mapping File and Convert Column Names
# =============================================================
...
def extract_signature_proteins(data, labels):
    ...
    for c in clusters:
        ...
        # Three-tier significance criteria
        ...
        for direction in ['Up', 'Down']:
            ...
            try:
                enr = gp.enrichr(
                    ...
                    outdir=None,
                    no_plot=True,
                    cutoff=1.0  # No pre-filtering at this stage
                )
                ...
                print(f"      -> Failed: {e}")
                # Fallback: retry using the extended list with a smaller library set
...
def gsea_prerank_analysis(all_signatures):
    ...
    for c, sig in all_signatures.items():
        ...
        try:
            ...
            # ---------- Display results by library ----------
            # Detect the library identifier column
...
# =============================================================
#  STEP 4: Enrichment Visualization (Improved)
# =============================================================
def plot_enrichment_summary(all_enrichment, gsea_results, gsea_by_library, labels):
    ...
    # --- A. ORA bubble plot for each cluster ---
    for c in clusters:
        for direction in ['Up', 'Down']:
